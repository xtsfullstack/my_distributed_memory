...
int rop_set_key(redisContext *conn, char *key, char *value)
{
...
int rop_get_key(redisContext *conn, char *key, char *value, int *valuelen)
{
	redisReply* r = (redisReply*)redisCommand(conn, "GET %s", key);
	if(r->type != REDIS_REPLY_STRING)
	{   
		printf("failed to execute command[%s]", "get key");
		freeReplyObject(r);
		redisFree(conn);
		return -1; 
	}   

	*valuelen = strlen(r->str);
	memcpy(value, r->str, *valuelen);
